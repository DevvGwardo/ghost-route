...
Review gate: APPROVED. `npm run typecheck` clean, `npm test` 23/23 green.
...
- Port 8787 on this Mac belongs to `cursor-api-proxy` — the server defaults to
  **8801**, client to **5174**.
- MapLibre worker is self-hosted at `client/public/gr-map-worker.mjs` (+
  ...
  with the dep optimizer), which silently leaves tiles unrendered. Re-copy both
  files from `node_modules/maplibre-gl/dist/` on upgrade, and restart the dev
  server — vite does not pick up new `public/` files without a restart.
- Tiles: CARTO Voyager (free, no key) + OpenStreetMap. Routing: OSRM demo (rate-limited, not for production).
  Geocoding: Nominatim.
